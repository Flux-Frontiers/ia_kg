#!/usr/bin/env python3
"""
ingest.py — Build per-book DocKGs for every book in ia_kg, register
...
def print_summary(
    ...
    print()
    print("╔" + thick + "╗")
    print(f"║  {'IA KG Ingest  —  Job Summary':<{W - 2}}║")
    print("╠" + thick + "╣")
    print(f"║{'':{W}}║")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Build, register, and corpus-add per-book DocKGs for ia_kg.",
    ...
    p.add_argument(
        "--force-register", action="store_true", help="Re-register even if already registered"
    )
